...
// FIX #19: static routes BEFORE /:id to avoid param capture
...
// GET /api/users/logs/activity  – admin
...
// GET /api/users/me/profile  – client
...
// PATCH /api/users/me/integrations
// FIX #10: apiSecret / consumerSecret should be encrypted before storage.
//   Use AES-256 via INTEGRATION_SECRET env var. If env not set, warn and store as-is.
const crypto = require('crypto');
const ALGO = 'aes-256-ctr';

function encryptSecret(text) {
  const key = process.env.INTEGRATION_SECRET;
  if (!key || key.length < 32) return text; // fallback — warn in logs
  if (!text) return text;
...
router.patch('/me/integrations', protect, async (req, res) => {
  try {
    ...
    const safeConfig = { ...config, connected: true };
    // Encrypt secrets
    if (platform === 'shopify'     && safeConfig.apiSecret)      safeConfig.apiSecret      = encryptSecret(safeConfig.apiSecret);
    if (platform === 'woocommerce' && safeConfig.consumerSecret) safeConfig.consumerSecret = encryptSecret(safeConfig.consumerSecret);

    const update = { [`integrations.${platform}`]: safeConfig };
...
// DELETE /api/users/me/integrations/:platform
router.delete('/me/integrations/:platform', protect, async (req, res) => {
  try {
    const { platform } = req.params;
    if (!['shopify','woocommerce'].includes(platform))
      return res.status(400).json({ success: false, message: 'Invalid platform' });
    const reset = { [`integrations.${platform}`]: { connected: false } };
...
router.patch('/:id/limits', protect, adminOnly, async (req, res) => {
  try {
    ...
    await user.save();
    await logActivity(req.user._id, 'admin', 'USER_LIMITS_UPDATE', 'User', user._id,
      { maxOrdersPerDay, codLimit }, req.ip);
...
// FIX #5: route registered BEFORE module.exports so it actually works
router.patch('/:id/set-password', protect, adminOnly, async (req, res) => {
  try {
    ...
    await user.save();
    await logActivity(req.user._id, 'admin', 'USER_PASSWORD_SET', 'User', user._id,
      { targetEmail: user.email }, req.ip);
